...
pew_tidy <- pew_religion %>% gather(key=income, value=freq,-religion) %>% 
...
new_df <- raw %>% pivot_longer(cols=wk1:wk76, names_to = 'week', values_to = 'rank', names_prefix = 'wk') %>% mutate(date = as.Date(date.entered)+weeks(as.numeric(week)-1))
```
...
tb_clean <- raw %>% gather(key=col_aux,value='casos',m014:fu) %>% separate(col = 'col_aux', into = c('sexo','edad'), sep=1) %>% mutate(sexo = as.factor(sexo),edad = as.factor(edad))
```
...
weather_clean <- raw %>% pivot_longer(cols=d1:d31, names_to = 'day', values_to = 'count', names_prefix = 'd') %>% drop_na(count) %>% pivot_wider(names_from=element, values_from=count) %>% unite(col=fecha, year,month, day,sep='-') %>% mutate(fecha = as.Date(fecha))
```
...
```{r }
planes.f <- planes %>% filter(!grepl('DOUGLAS',manufacturer)) %>% filter(seats >= 90 & seats <= 210) %>% mutate(manufacturer = ifelse(str_detect(manufacturer, 'AIRBUS'),'AIRBUS COMP', manufacturer))
```
...
```{r }
flights <- flights %>% select(-time_hour,-ends_with('time'))
...
```{r }
flights <- flights %>% mutate(Full_Date = paste(day,month,year,sep='-')) %>% select(-year,-month,-day)
```
...
```{r }
Distances_plane <- flights %>% group_by(tailnum) %>% 
  summarise(Num_flights = n(), Mean_distance = mean(distance), 
            Total_distance = sum(distance)) %>% drop_na(tailnum)
```
...
df <- raw %>% pivot_longer(cols=wk1:wk76, names_to = 'week', values_to = 'rank', names_prefix = 'wk') %>% mutate(date = as.Date(date.entered)+weeks(as.numeric(week)-1))

uniq_tracks <- unique(df$track)
id <- 1:length(uniq_tracks)

df$id <- as.numeric(factor(df$track,levels=uniq_tracks,labels=id))

new_df <- data.frame(id = id,tracks = uniq_tracks)
...
Primero unimos las columnas century y year en una sola.
```{r}
t5t <- t5 %>% mutate(year=paste0(century,year)) %>% select(-century) %>%
...
```{r}
df <- Distances_plane %>% full_join(planes, by='tailnum') %>%  select(c('tailnum','Total_distance','manufacturer','model')) %>% arrange(-Total_distance)
```
...
```{r}
df %>% full_join(flights, by='tailnum') %>% group_by(manufacturer) %>% summarise(mean_dep_delay=mean(dep_delay,na.rm=T),
            mean_arr_delay=mean(arr_delay,na.rm=T))
```

La compañía con los máximos retrasos medios de salida es BOMBARDIER INC, la cual también es la compañía con los máximos retrasos de llegada. Por tanto, concluimos que BOMBARDIER INC es, según los datos proporcionados, la compañía aérea con mayores retrasos medios, tanto de llegada como de salida.
